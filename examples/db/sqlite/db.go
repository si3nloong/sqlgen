...
func FindByPK[T sequel.KeyValuer, Ptr sequel.KeyValueScanner[T]](ctx context.Context, sqlConn *sql.DB, model Ptr) error {
	...
	case sequel.CompositeKeyer:
		...
		return sqlConn.QueryRowContext(ctx, "SELECT "+strings.Join(TableColumns(model), ",")+" FROM "+DbTable(model)+" WHERE ("+strings.Join(keyNames, ",")+") = (?"+strings.Repeat(",?", len(keyNames)-1)+") LIMIT 1;", keys...).Scan(model.Addrs()...)
	default:
		panic("unreachable")
	}
}
