...
func (AutoPkLocation) Schemas() sequel.TableDefinition {
	return sequel.TableDefinition{
		...
		Columns: []sequel.ColumnDefinition{
			...
			{Name: "geo_point", Definition: "geo_point POINT NOT NULL"},
			{Name: "ptr_geo", Definition: "ptr_geo VARCHAR(255)"},
			{Name: "ptr_date", Definition: "ptr_date DATE"},
		},
	}
}
...
func (v *AutoPkLocation) Addrs() []any {
	return []any{types.Integer(&v.ID), ewkb.Scanner(&v.GeoPoint), types.JSONUnmarshaler(&v.PtrGeo), types.JSONUnmarshaler(&v.PtrDate)}
}
...
func (v AutoPkLocation) UpdateOneByPKStmt() (string, []any) {
	return "UPDATE auto_pk_location SET geo_point = ST_GeomFromEWKB(?),ptr_geo = ?,ptr_date = ? WHERE id = ? LIMIT 1;", []any{ewkb.Value(v.GeoPoint, 4326), types.JSONMarshaler(v.PtrGeo), types.TextMarshaler(v.PtrDate), int64(v.ID)}
}
